        ...
        for topic, msgs in rosbag_msgs.items():
            if topic == "/map_server/map_filtered":
                ...
                if not log_data.total_path:
                    ...
                    m = len(log_data.total_path) - \
                        len(log_data.paths[drone_id])
                    new_path_length = m*[0]+log_data.paths[drone_id]
                    log_data.total_path = np.add(
                        log_data.total_path, new_path_length).tolist()
            elif "pose" in topic:
...
def plot_total_path(data: LogData, fig: plt.Figure = None) -> plt.Figure:
    ...
    if len(data.timestamps) != len(data.total_path):
        data.timestamps = data.timestamps[:len(data.total_path)]
...
def plot_path(data: LogData):
    ...
    for k, v in data.paths.items():
        ts = data.timestamps[:len(v)]
        ax.plot(ts, v, label=k)
