...
class Explorer(DroneInterface):
    ...
    def __init__(self, drone_id: str = "drone0", verbose: bool = False,
        ...
        super().__init__(f"fake_explorer_{drone_id}", verbose, use_sim_time)
        self.namespace = drone_id

        self.explore_client = self.create_client(
            SetBool, f"/{drone_id}/start_exploration")
        ...
        self.twist_sub = self.create_subscription(
            TwistStamped, f'/{drone_id}/self_localization/twist', self.twist_cbk, qos_profile_sensor_data)

        # Overriding pose methods to republish to evaluator
        self.__pose = PoseData()
        self.pose_sub = self.create_subscription(
            PoseStamped, f'/{drone_id}/self_localization/pose', self.pose_cbk, qos_profile_sensor_data)
